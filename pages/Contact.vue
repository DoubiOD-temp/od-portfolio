    ...
    <div class="contact-stack">
      <div class="card">
        <video
          ...
          playsinline
          preload="metadata"
          poster="/contact-orb-poster.jpg"
        >
          <source src="/contact-orb.webm" type="video/webm" />
          <source src="/contact-orb.mp4" type="video/mp4" />
        </video>
        ...
        >
          <span v-for="(word, wIdx) in taglineWords" :key="`word-${wIdx}`" class="word">
            <span
              v-for="charObj in word"
              :key="`char-${charObj.idx}`"
              class="tagline-char"
              :class="{ revealed: charObj.idx < revealedChars }"
            >{{ charObj.ch }}</span>
          </span>
        </p>
        ...
        <div class="footer-row">
          ...
          <div class="socials-row">
            <a href="https://www.linkedin.com/in/ondrej-dobis/" target="_blank" rel="noopener" aria-label="LinkedIn">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#0A66C2" width="36" height="36" aria-hidden="true">
            ...
            <a href="https://github.com/DoubiOD-temp" target="_blank" rel="noopener" aria-label="GitHub (work)">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#181717" width="36" height="36" aria-hidden="true">
      ...
      <div class="actions" ref="actionsRef">
        <a class="btn btn-primary" href="/CV/Dobis_CV_0626.pdf" target="_blank" rel="noopener">
          Download CV
        </a>
        <a class="btn btn-secondary" href="/CV/Ondrej RL2024.pdf" target="_blank" rel="noopener">
...
let currentWord: { ch: string; idx: number }[] = [];

taglineChars.forEach((ch, idx) => {
  currentWord.push({ ch, idx });
  if (ch === ' ') {
    taglineWords.push(currentWord);
    currentWord = [];
  }
});
...
onMounted(() => {
  ...
  window.setTimeout(() => {
    typewriterTimer = setInterval(() => {
      if (revealedChars.value >= taglineChars.length) {
        if (typewriterTimer) clearInterval(typewriterTimer);
        typewriterTimer = null;
        return;
      }
      revealedChars.value += 1;
    }, 22);
  }, 120);

  if (actionsRef.value) {
    gsap.fromTo(
      actionsRef.value.querySelectorAll('.btn'),
      { y: -60, opacity: 0 },
